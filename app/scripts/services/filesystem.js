...
angular.module('FileBroWSApp')
    .factory('filesystem', function ($q,$http) {
        var root = "/fs";
        window.filebro.registerMimeType = function(key, val) {

        };
        window.filebro.events.getFile = function(data, action) {
            var listeners = window.filebro.events.getFile.listeners;
            window.filebro.events.getFile.listeners = listeners || [];
            window.filebro.events.getFile.listeners.push({
                type: data.type,
                action : action
            });
        };
        function parseDir(dir) {
            return $http.get(root + dir).then(function (data) {
                var $data = $(data.data);
                var $list = $data.find("li");
                var arr = [];
                $.each($list, function (key, el) {
                    var name = el.textContent;
                    var isDir = (name.indexOf("/") > -1) || (name.indexOf("@") > -1);
                    var type;
                    var ext = "";
                    if (!isDir) {
                        ext = name.substring(name.lastIndexOf(".") + 1, name.length).replace('\n', "");
                        type = api.mimeTypes.lookup(name.substring(0, name.length-1));
        ...
        var api = {
            ...
            getFile : function(dir) {
                ...
                return $http.get(root+dir).then(function(data) {
                    if(listeners && listeners.length) {
                        var mime = api.mimeTypes.lookup(dir);
                        if(mime) {
                           angular.forEach(listeners, function(listener) {
                                if(mime.indexOf(listener.type)>-1) {
                                    data = listener.action(data);
                                }
                           });
                        }
                    }
            ...
            openFile: function(file) {
                ...
                if(listeners) {
                    var isHandled = false;
                    angular.forEach(listeners, function(listener) {
                        if(file.type.indexOf(listener.type) > -1) {
                            isHandled = true;
                            file.get = function() {
                                return api.getFile(file.path);
                            };
                            defer.resolve(listener.action(file));
                        }
                    });
                    if(!isHandled) {
                        defer.reject({ message :"No registered listeners for this type"});
                    }
                } else {
                    defer.reject({ message :"No registered listeners"});
                }
